...
public class VerticallyJumpingGraphPathFinder implements PathFinder {
    private final double jumpCost;

    public VerticallyJumpingGraphPathFinder(double jumpCost) {
    ...
    public int[] findPath(ReadableImage<WrappedDouble> dsi) {
        ...
        for (int l = 1; l < n; l++){
            for (int r = 0; r < n; r++) {
                ...
                for (int j = 0; j <= r; j++){
                    double cost = costs[l-1][j] + costHere + Math.abs(r-j-1)*jumpCost;
...

        {
            ...
            for (int j = 0; j < n; j++){
                double cost = costs[n-1][j] + (n-j-1)*(n-j-1)*jumpCost;
        ...
        int lastVal = result[0];
        for (int i=1 ;i< n-1; i++){
            if (result[i] == lastVal && result[i+1] == lastVal){
                result [i] = -1;
            } else {
                lastVal = result[i];
            }
